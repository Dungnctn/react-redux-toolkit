import { ArrowRightOutlined, DeleteOutlined, DoubleRightOutlined, DownloadOutlined } from '@ant-design/icons';
    ...
    const dataSource = orders?.map((item, index) => {
        return {
            ...
            status: item?.status === 1 ? <div> <span className="bg-green-500 text-white p-2 rounded-lg">Đã gửi đơn</span></div> 
                                        : <div> <span className="bg-red-500 text-white p-2 rounded-lg">Chưa gửi đơn</span></div>,
            detail: <NavLink to={`${item._id}/detail`}> Detail Order  <Button type="primary" shape="round" icon={<ArrowRightOutlined />}></Button ></NavLink>,
        }
    })
