...
const Checkout = () => {
  ...
  return (
    <div>
      <Header />
      {!user && <div className='mt-20 text-white text-lg'>
        <div className='bg-cyan-600 h-10'>
          <span className='leading-10'>Bạn cần đăng nhập để đặt hàng <NavLink to='/signin' className="text-black"><u>Đăng nhập</u></NavLink> </span>
          <br />
          <button className='btn-cart'><NavLink to={'/'} className='home'>Home</NavLink>
          </button>
        </div>
      </div>}
      {(cart.cartItems.length === 0) && user && <div className='mt-20 text-white text-lg'>
        <div className='bg-cyan-600 h-10'>
          <span className='leading-10'>Bạn chưa có sản phẩm trong giỏ hàng <NavLink to='/cart' className="text-black"><u>Giỏ hàng</u></NavLink> </span>
        </div>
      </div>}
        ...
        <ol role="list" class="bg-[#F1F1F1] max-w-2xl mt-10 mx-auto px-4 py-2 flex items-center space-x-2 sm:px-6 lg:max-w-7xl lg:px-8">
          ...
          <li>
            <div class="flex items-center">
                ...
                <svg width="16" height="20" viewBox="0 0 16 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" class="w-4 h-5 text-gray-300">
                <path d="M5.697 4.34L8.98 16.532h1.327L7.025 4.341H5.697z" /></svg>
            </div>
          </li>
          <li>
            <div class="flex items-center">
                <NavLink to={"/checkout"} class="mr-2 text-sm font-medium text-gray-900">
                  Đặt hàng
                </NavLink>
            </div>
          </li>
                      ...
                      <div class="">
                          ...
                          <div class="flex items-center mt-4">
                              <label class="flex items-center text-sm group text-heading">
                                  ...
                                  <span class="ml-2">Lưu thông tin cho lần đặt hàng sau</span></label>
                          </div>
                          ...
                          <div class="mt-4">
                              <button
                                  class="w-full px-6 py-2 text-blue-200 bg-blue-600 hover:bg-blue-900">Đặt hàng</button>
                          </div>
                      </div>
                    </form>
                </div>
                              ...
                              {cart.cartItems.map((item) => 
                                <div class="flex space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <span class="text-green-600">Số lượng:</span> {item.quantity} <br/>
                                        <span class="text-red-600">Giá:</span> {(item.price).toLocaleString('vn-VN')} <u>đ</u>
                                    </div>
                                    <div>
                                      <button onClick={() => handleRemove(item)} title='Xóa'>
